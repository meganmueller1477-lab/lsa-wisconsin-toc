...
export default function SiteInfoPage() {
  ...
  useEffect(() => {
    (async () => {
      const q1 = query(collection(db, "pages"), where("type", "==", "siteinfo"), orderBy("title", "asc"));
  ...
  return (
    <main className="space-y-4">
      <header className="rounded-3xl border border-white/20 bg-white/5 p-4">
        <h1 className="text-xl font-extrabold">Site Info</h1>
        <p className="text-sm text-white/70 mt-1">Locations, apparel, schedules, and details.</p>
      </header>
